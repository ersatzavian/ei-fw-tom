...
// Pin 2: SPI (MISO)
// Pin 5: SPI (SCLK)
// Pin 6: LCD Reset (active low)
// Pin 7: SPI (MOSI)
// Pin 8: LCD Chip Select (active low)
// Pin 9: LCD Mode - Data (high) / Command (low)
hardware.pin1.configure(PWM_OUT, 0.002, 1.0);
hardware.pin6.configure(DIGITAL_OUT);
hardware.pin6.write(0);
...
screen <- ST7735_LCD(128, 160, hardware.spi257, hardware.pin1, hardware.pin6, hardware.pin8, hardware.pin9);
...
// After receiving part of an image, write to the screen and check for more chunks
agent.on("imageChunk", function(data) {
    server.log("loading image chunk " + data.thisChunk);
    screen.writeBlob(data.chunkBlob);                        // Write chunk to screen
