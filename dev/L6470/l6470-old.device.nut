...
class L6470 {
  ...
  function dspinXfer(data) {
    ...
    local resultStr = spi.writeread(format("%c",data));
    imp.sleep(0.0001);
    cs_l.write(1);
  ...
  constructor(_spi, _cs_l, _alert_l, _rst_l) {
    ...
    cs_l.write(1);
    alert_l.configure(DIGITAL_IN_PULLUP,handleFlag.bindenv(this));
  ...
  function getParam(param) {
    server.log(format("getParam: asking for 0x%02x",param));
    server.log(format("getParam: full cmd is 0x%02x",(dSPIN_GET_PARAM | param)));
